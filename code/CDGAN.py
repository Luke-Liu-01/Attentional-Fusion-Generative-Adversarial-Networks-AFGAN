...
class NetG(nn.Module):
    def __init__(self, nz, ngf, channels_img = 3, embedding_dim = 300):
        ...
        self.generator = nn.Sequential(
            # state size: nz * 1 * 1 -> ngf*16 * 4 * 4
            # self.tc_block(in_channels=nz, out_channels=ngf*8, kernel_size=4, stride=1, padding=0, bias=False),

            # state size: ngf*8 * 4 * 4 -> ngf*4 * 8 * 8
            self.tc_block(in_channels=ngf*8, out_channels=ngf*4, kernel_size=4, stride=2, padding=1, bias=False),

            # state size: ngf*4 * 8 * 8 -> ngf*2 * 16 * 16
            self.tc_block(in_channels=ngf*4, out_channels=ngf*2, kernel_size=4, stride=2, padding=1, bias=False),

            # state size: ngf*2 * 16 * 16 -> ngf*1 * 32 * 32
            self.tc_block(in_channels=ngf*2, out_channels=ngf*1, kernel_size=4, stride=2, padding=1, bias=False),

            # state size: ngf*1 * 32 * 32 -> channels_img * 64 * 64
            nn.ConvTranspose2d(in_channels=ngf*1, out_channels=channels_img, kernel_size=4, stride=2, padding=1, bias=False),
            nn.Tanh()  # [-1, 1]
        )
    
    # Transposed Convolution + Batch-Normalization
    def tc_block(self, in_channels, out_channels, kernel_size, stride, padding, bias):
...
class NetD(nn.Module):
    def __init__(self, nz, ndf, channels_img = 3, embedding_dim = 300):
        ...
        self.cap_dim_reduce = nn.Sequential(
            nn.Linear(in_features=embedding_dim, out_features=nz),
            # nn.BatchNorm1d(nz),  # cite
            nn.ReLU()
        )
        ...
        self.discriminator = nn.Sequential(
            ...
            self.c_block(in_channels=ndf*4, out_channels=ndf*8, kernel_size=3, stride=2, padding=1, bias=False),
